...
from models.networks import NGP_v2
...
class NeRFSystem(LightningModule):
    def __init__(self, hparams):
        ...
        self.model = NGP_v2(scale=self.hparams.scale,
                            hparams=hparams,
                            rgb_act=rgb_act)
    ...
    def render_virtual_cam_train(self,new_c2w, batch, pix_idxs):
        v_batch = {'pose': new_c2w.to(batch['pose']),
                   'img_idxs': batch['img_idxs'],
                   'pix_idxs': pix_idxs}
        v_results = self(v_batch, split='train')
        return v_results
    ...
    def beta_uncert(self, results, isdense=True):
        ...
        if not isdense:
            if len(opacity) > len(pix_ids):
                opacity = opacity[pix_ids]
            if len(betas) > len(pix_ids):
                betas = betas[pix_ids]

        nan_mask = torch.isnan(betas)
        betas[nan_mask] = 0
        counts = (opacity > 0) & (betas > 0)
        beta_score = torch.nanmean(betas[counts].flatten())
        return betas.cpu(), counts.cpu(), beta_score.cpu()
    ...
    def validation_step(self, batch, batch_nb):
        ...
        if self.hparams.eval_u:
            ...
            for u_method in self.hparams.u_by:
                ...
                if u_method == 'beta':
                    sigmas, counts, u_score = self.beta_uncert(results, isdense=dense_tag)
                    counts = counts.to(torch.float)
