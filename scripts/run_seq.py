...
def main(args):
	...
	if n_steps > 0:
		with tqdm(desc="Training", total=n_steps, unit="step") as t:
			while True:
				while testbed.frame():
					if testbed.is_train_aborted == True:
						break
					# # GPU warmup
					# optimizer.zero_grad()
	...
	return t_train_time, testbed.loss, testbed.n_params(), testbed.n_encoding_params(), n_train_aborting_count, loss_data, seed

if __name__ == "__main__":
	args = parse_args()
	## Ablation test varying density and rgb network hidden layer and channels.
	for n_dense_hidden in range(1, 2):
		for n_rgb_hidden in range(1, 4):
			# for n_channel in [16, 32, 64, 128, 256]:
			for n_channel in [16, 32, 64, 128]:
				args.save_snapshot = "snapshots/lego/fig10/base_{}layer_c{}.msgpack".format(n_rgb_hidden, n_channel)
				args.network = "yj/fig10/base_{}layer_c{}.json".format(n_rgb_hidden, n_channel)
				t_train_time, loss, n_params, n_encoding_params, n_train_aborting_count, loss_data, seed = main(args)

				with open('fig10_test_T19.csv', 'a') as f:
					w = csv.writer(f)
					w.writerow(['T19_d{}c{}h_r{}c{}h'.format(n_channel, n_dense_hidden, n_channel, n_rgb_hidden), t_train_time, loss, n_params, n_encoding_params, n_train_aborting_count, seed])

				with open('/home/ylee298/instant-ngp/outputs/nerf/lego/fig10/base_{}layer_c{}.csv'.format(n_rgb_hidden, n_channel), 'w') as f:
					w = csv.writer(f)
					w.writerows(zip(loss_data))

	
	# for n_dense_hidden in range(1, 2):
	# 	for n_rgb_hidden in range(1, 4):
	# 		for n_channel in [16, 32, 64, 128]:
	# 			args.load_snapshot = "snapshots/lego/fig10/base_{}layer_c{}.msgpack".format(n_rgb_hidden, n_channel)
	# 			if os.path.exists(args.load_snapshot):
	# 				args.network = "yj/fig10/base_{}layer_c{}.json".format(n_rgb_hidden, n_channel)
	# 				psnr, minpsnr, maxpsnr, ssim = main(args)
				
	# 				with open('fig10_result_T19.csv', 'a') as f:
	# 					w = csv.writer(f)
	# 					w.writerow(['T19_d{}c{}h_r{}c{}h'.format(n_channel, n_dense_hidden, n_channel, n_rgb_hidden), psnr, minpsnr, maxpsnr, ssim])