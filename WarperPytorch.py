...
class Warper:
    ...
    def forward_warp_depth(self, mask1: Optional[torch.Tensor], depth1: torch.Tensor,
                     transformation1: torch.Tensor, transformation2: torch.Tensor, intrinsic1: torch.Tensor,
                     intrinsic2: Optional[torch.Tensor]) -> \
        ...
        if mask1 is None:
            mask1 = torch.ones(size=(b, 1, h, w)).to(depth1)
        if intrinsic2 is None:
            intrinsic2 = intrinsic1.clone()
        ...
        trans_depth1 = trans_points1[:, :, :, 2, 0]
        trans_depth1 = trans_depth1.unsqueeze(1)

        grid = self.create_grid(b, h, w).to(trans_coordinates) # (b, 2, h , w )

        flow12 = trans_coordinates - grid
        warped_depth2, _ = self.bilinear_splatting(trans_depth1, mask1, trans_depth1, flow12, None,
                                                is_image=False)
        return warped_depth2, flow12
